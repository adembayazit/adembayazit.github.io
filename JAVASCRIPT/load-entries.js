   // 1. GLOBAL DEĞİŞKENLER
        ...
        // İçerik işleme fonksiyonu - yeni satırları <br> ile değiştirir
        function processContent(content) {
            if (!content) return '';
            
            // Önce YouTube embed'lerini işle
            content = processYoutubeEmbeds(content);
            
            // Sonra Spotify embed'lerini işle
            content = processSpotifyEmbeds(content);
            
            // Son olarak yeni satırları <br> ile değiştir
            return content.replace(/\n/g, '<br>');
        }

        // YouTube embed işleme
        function processYoutubeEmbeds(content) {
            const youtubeRegex = /<div class="youtube-embed">.*?<iframe.*?src="https:\/\/www\.youtube\.com\/embed\/([a-zA-Z0-9_-]+)".*?><\/iframe><\/div>/g;
        ...
        // Spotify embed işleme
        function processSpotifyEmbeds(content) {
            const spotifyRegex = /<iframe.*?class="spotify-embed".*?src="https:\/\/open\.spotify\.com\/embed\/track\/([a-zA-Z0-9]+).*?"><\/iframe>/g;
            return content.replace(spotifyRegex, (match, trackId) => {
                return `<iframe 
                    class="spotify-embed"
                ...
                icon.addEventListener("click", (e) => {
                    ...
                    if (icon.classList.contains("active")) {
                        if (translationEntry.content_tr) {
                            contentDiv.innerHTML = processContent(translationEntry.content_tr);
                        }
                    } else {
        ...
        function createEntryElement(entry, container, depth) {
            ...
            // İçeriği işle (yeni satırları <br> ile değiştir)
